...
function dashboardView() {
  ...
  return `<div class="page-heading"><div><p class="eyebrow">Lundi 12 juin 2024</p><h1>Bonjour ${escapeHtml(currentUserName())}, <span class="wave">✦</span></h1><p class="subtle">Voici ce qui se passe dans vos magasins aujourd’hui.</p></div><button class="btn btn-primary" data-action="new-sale">＋ Nouvelle vente</button></div>
  <section class="stats-grid">
    <article class="glass-card stat-card"><div class="stat-top"><span>Ventes du jour</span><span class="stat-symbol">↗</span></div><h2>${money(totalSales)}</h2><div class="stat-foot"><b>+12,8%</b> vs. hier</div></article>
    <article class="glass-card stat-card"><div class="stat-top"><span>Transactions</span><span class="stat-symbol">≋</span></div><h2>${transactionCount}</h2><div class="stat-foot neutral">Depuis l’ouverture du jour</div></article>
    <article class="glass-card stat-card"><div class="stat-top"><span>Dépenses du jour</span><span class="stat-symbol">−</span></div><h2>${money(totalExpenses)}</h2><div class="stat-foot neutral">2 dépenses enregistrées</div></article>
    <article class="glass-card stat-card"><div class="stat-top"><span>${treasurySnapshot ? 'Trésorerie attendue' : 'Solde en caisse'}</span><span class="stat-symbol">▣</span></div><h2>${money(cashExpected)}</h2><div class="stat-foot ${treasurySnapshot?.cash?.open ? '' : 'neutral'}">${treasurySnapshot?.cash?.open ? 'Caisse ouverte' : 'Caisse non ouverte'}</div></article>
  </section>
...
function genericHeader(title, subtitle, action, actionLabel) { return `<div class="page-heading"><div><p class="eyebrow">Gestion opérationnelle</p><h1>${title}</h1><p class="subtle">${subtitle}</p></div>${action ? `<button class="btn btn-primary" data-action="${action}">＋ ${actionLabel}</button>` : ''}</div>`; }
...
function reportsView() { return genericHeader('Rapports', 'Analysez les résultats par période, magasin ou vendeur.', null, '') + `<section class="stats-grid"><article class="glass-card stat-card"><div class="stat-top"><span>Ticket moyen</span><span class="stat-symbol">⌁</span></div><h2>${money(3162)}</h2><div class="stat-foot"><b>+6,2%</b> ce mois</div></article><article class="glass-card stat-card"><div class="stat-top"><span>Liquide</span><span class="stat-symbol">₣</span></div><h2>61%</h2><div class="stat-foot neutral">des paiements</div></article><article class="glass-card stat-card"><div class="stat-top"><span>Meilleur magasin</span><span class="stat-symbol">★</span></div><h2>Plateau</h2><div class="stat-foot"><b>+18,4%</b> vs. Médina</div></article><article class="glass-card stat-card"><div class="stat-top"><span>Écart caisse</span><span class="stat-symbol">≠</span></div><h2 class="positive">+ 0 FCFA</h2><div class="stat-foot neutral">Sur les 7 derniers jours</div></article></section><section class="glass-card panel"><div class="panel-header"><div><h3>Résumé des performances</h3><p>Comparaison par magasin</p></div><select class="filter-input"><option>Ce mois</option><option>Cette semaine</option></select></div>${salesTable(data.sales)}</section>`; }
