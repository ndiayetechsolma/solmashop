...
let cashHistoryByStore = {};
let cashHistoryLoading = false;
...
async function loadRemoteDataLegacy() {
  ...
  try {
    ...
    data.sales = salesResult.data.map(item => ({ id: item.id, timestamp: item.date_heure, cancelled: item.annulee, product: item.nom_produit, amount: item.montant, payment: item.mode_paiement, seller: item.personnel?.nom || item.admin_nom || 'Administrateur', store: item.magasins?.nom || 'Magasin', time: toTime(item.date_heure) }));
    data.expenses = expensesResult.data.map(item => ({ id: item.id, timestamp: item.date_heure, cancelled: item.annulee, reason: item.motif, amount: item.montant, addedBy: item.personnel?.nom || item.admin_nom || 'Administrateur', store: item.magasins?.nom || 'Magasin', time: toTime(item.date_heure) }));
...
async function loadRemoteData() {
  ...
  try {
    ...
    data.sales = (payload.sales || []).map(item => ({ id: item.id, timestamp: item.date_heure, cancelled: item.annulee, product: item.nom_produit, amount: item.montant, payment: item.mode_paiement, seller: item.personnel?.nom || item.admin_nom || 'Administrateur', store: item.magasins?.nom || 'Magasin', time: toTime(item.date_heure) }));
    data.expenses = (payload.expenses || []).map(item => ({ id: item.id, timestamp: item.date_heure, cancelled: item.annulee, reason: item.motif, amount: item.montant, addedBy: item.personnel?.nom || item.admin_nom || 'Administrateur', store: item.magasins?.nom || 'Magasin', time: toTime(item.date_heure) }));
...
async function loadCashHistory() {
  if (cashHistoryLoading) return;
  cashHistoryLoading = true;
  try {
    if (window.solmaPersonnelSession) {
      const token = window.solmaPersonnelSession.token;
      const storeId = window.solmaPersonnelSession.personnel.magasin_id;
      const result = await fetch(`/api/cash?magasin_id=${storeId}&history=true`, { headers: { Authorization: `Bearer ${token}` } });
      if (result.ok) { const payload = await result.json(); cashHistoryByStore[treasurySnapshot?.store || storeId] = payload.history || []; }
    } else {
      const token = (await window.solmaSupabase.auth.getSession()).data.session?.access_token;
      await Promise.all(stores.map(async store => {
        const result = await fetch(`/api/cash?magasin_id=${store.id}&history=true`, { headers: { Authorization: `Bearer ${token}` } });
        if (result.ok) { const payload = await result.json(); cashHistoryByStore[store.nom] = payload.history || []; }
      }));
    }
  } finally {
...
function dashboardView() {
  ...
  const todayExpenses = getFiltered(data.expenses).filter(expense => isToday(expense.timestamp) && !expense.cancelled);
  ...
  const openCashStores = getFiltered(data.cash).filter(c => c.opened);
const cashExpected = treasurySnapshot?.cash?.expected ?? openCashStores.reduce((sum, c) => sum + (c.expected || 0), 0);
const anyCashOpen = treasurySnapshot ? Boolean(treasurySnapshot.cash?.open) : openCashStores.length > 0;
  ...
  <section class="stats-grid">
    <article class="glass-card stat-card"><div class="stat-top"><span>Ventes du jour</span><span class="stat-symbol">${icon('trendingUp')}</span></div><h2 data-count-target="${totalSales}" data-count-format="money">${money(0)}</h2><div class="stat-foot ${yesterdayTotal > 0 ? '' : 'neutral'}"><b>${trendLabel}</b> vs. hier</div></article>
    <article class="glass-card stat-card"><div class="stat-top"><span>Transactions</span><span class="stat-symbol">${icon('activity')}</span></div><h2 data-count-target="${transactionCount}" data-count-format="int">0</h2><div class="stat-foot neutral">Depuis l’ouverture du jour</div></article>
    <article class="glass-card stat-card"><div class="stat-top"><span>Dépenses du jour</span><span class="stat-symbol">${icon('trendingDown')}</span></div><h2 data-count-target="${totalExpenses}" data-count-format="money">${money(0)}</h2><div class="stat-foot neutral">${todayExpenses.length} dépense${todayExpenses.length > 1 ? 's' : ''} enregistrée${todayExpenses.length > 1 ? 's' : ''}</div></article>
    <article class="glass-card stat-card"><div class="stat-top"><span>${treasurySnapshot ? 'Trésorerie attendue' : 'Solde en caisse'}</span><span class="stat-symbol">${icon('wallet')}</span></div><h2 data-count-target="${cashExpected}" data-count-format="money">${money(0)}</h2><div class="stat-foot ${anyCashOpen ? '' : 'neutral'}">${anyCashOpen ? 'Caisse ouverte' : 'Caisse non ouverte'}</div></article>
  </section>
  ...
  <article class="glass-card panel"><div class="panel-header"><div><h3>État des caisses</h3><p>Suivi en temps réel</p></div><button class="text-link" data-view-link="cash">Tout voir</button></div><div class="cash-list">${getFiltered(data.cash).map(cash => `<div class="cash-item"><div class="store-icon">${icon('store', 18)}</div><div class="cash-info"><strong>${cash.store}</strong><span>${cash.opened ? (cash.openedAt && cash.openedAt.includes(', ') ? 'Ouverte à ' + cash.openedAt.split(', ')[1] : 'Ouverte') : 'Fermée'}</span></div><div class="cash-amount"><strong>${money(cash.expected)}</strong><span class="${cash.opened ? '' : 'closed'}">${cash.opened ? 'En cours' : 'Fermée'}</span></div><div class="cash-progress"><i class="${cash.opened ? '' : 'closed'}"></i></div></div>`).join('')}</div></article></section>
...
function expensesView() {
  const admin = !window.solmaPersonnelSession;
  const rows = getFiltered(data.expenses).map((item, index) => `<tr class="${item.cancelled ? 'cancelled-row' : ''}"><td><strong>${escapeHtml(item.reason)}</strong></td><td>${escapeHtml(item.addedBy)}</td><td class="muted">${item.store}</td><td class="negative"><strong>− ${money(item.amount)}</strong></td><td class="muted">${item.time}${item.cancelled ? ' · Annulée' : ''}</td>${admin ? `<td>${item.cancelled ? '<span class="muted">Annulée</span>' : `<button class="text-link danger-link" data-cancel-expense="${item.id || ''}" data-cancel-expense-local="${index}">Annuler</button>`}</td>` : ''}</tr>`).join('');
  return genericHeader('Dépenses', 'Gardez une trace claire des sorties de chaque magasin.', 'new-expense', 'Ajouter une dépense') + `<section class="glass-card view-card">${data.expenses.length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Motif</th><th>Ajoutée par</th><th>Magasin</th><th>Montant</th><th>Date</th>${admin ? '<th>Action</th>' : ''}</tr></thead><tbody>${rows}</tbody></table></div>` : '<div class="empty-state"><strong>Aucune dépense</strong>Les dépenses ajoutées apparaîtront ici.</div>'}</section>`;
}
function cashView() {
  let items;
  if (stores.length) {
  ...
  const currentCards = items.length ? items.map(cash => `<article class="glass-card view-card"><div class="panel-header"><div><h3>Caisse ${escapeHtml(cash.store)}</h3><p>${cash.opened ? 'Ouverte' : 'Fermée'}${cash.openedBy ? ' · ' + escapeHtml(cash.openedBy) : ''}</p></div><span class="badge ${cash.opened ? 'badge-money' : 'badge-cash'}">${cash.opened ? 'En cours' : 'Fermée'}</span></div><div class="form-grid"><div><span class="muted">Montant d'ouverture</span><h3>${money(cash.opening)}</h3></div><div><span class="muted">Montant attendu</span><h3>${money(cash.expected)}</h3></div></div><div class="form-actions">${!cash.opened && cash.expected ? `<button class="btn btn-light" data-action="reopen-cash" data-store="${escapeHtml(cash.store)}">Rouvrir la dernière caisse</button>` : ''}<button class="btn ${cash.opened ? 'btn-danger' : 'btn-primary'}" data-action="${cash.opened ? 'close-cash' : 'open-cash'}" data-store="${escapeHtml(cash.store)}">${cash.opened ? 'Fermer la caisse' : 'Ouvrir la caisse'}</button></div></article>`).join('') : '';
  const selectedStore = document.querySelector('#store-filter')?.value || 'all';
  const relevantStoreNames = selectedStore === 'all' ? Object.keys(cashHistoryByStore) : Object.keys(cashHistoryByStore).filter(name => name.toLowerCase() === selectedStore || name.toLowerCase().includes(selectedStore));
  const pastRows = relevantStoreNames.flatMap(storeName => (cashHistoryByStore[storeName] || []).filter(row => row.date_fermeture).map(row => ({ ...row, storeName }))).sort((a, b) => new Date(b.date_fermeture) - new Date(a.date_fermeture)).slice(0, 30).map(row => {
  const label = new Date(row.date_fermeture).toLocaleDateString('fr-FR', { weekday: 'short', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' });
  const ecart = row.ecart || 0;
    return `<tr><td><strong>${label}</strong></td><td class="muted">${escapeHtml(row.storeName)}</td><td>${money(row.montant_ouverture)}</td><td>${money(row.montant_fermeture)}</td><td class="${ecart >= 0 ? 'positive' : 'negative'}">${ecart >= 0 ? '+ ' : '− '}${money(Math.abs(ecart))}</td></tr>`;
  }).join('');
  return genericHeader('Caisses', 'Ouvertures, fermetures et écarts de caisse.', null, '') +
    (currentCards ? `<section class="cash-list">${currentCards}</section>` : `<section class="glass-card view-card"><div class="empty-state"><strong>Aucune caisse enregistrée</strong><span>Ouvrez une caisse pour commencer le suivi du jour.</span></div></section>`) +
    `<section class="glass-card panel" style="margin-top:14px"><div class="panel-header"><div><h3>Historique des jours passés</h3><p>Toutes les caisses clôturées, conservées indéfiniment</p></div></div>${pastRows ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Clôturée le</th><th>Magasin</th><th>Ouverture</th><th>Fermeture</th><th>Écart</th></tr></thead><tbody>${pastRows}</tbody></table></div>` : '<div class="empty-state"><strong>Pas encore d\'historique</strong><span>Il apparaîtra ici après la première fermeture de caisse.</span></div>'}</section>`;
}
...
function reportsView() {
  ...
  expenses.forEach(expense => { const key = dayKey(expense.timestamp); days[key] = days[key] || { sales: 0, expenses: 0 }; days[key].expenses += expense.amount; });
  const sortedDays = Object.keys(days).sort((a, b) => b.localeCompare(a));
  const dailyRows = sortedDays.map(key => {
    const { sales: s, expenses: e } = days[key];
    const label = new Date(key).toLocaleDateString('fr-FR', { weekday: 'short', day: '2-digit', month: 'short' });
    return `<tr><td><strong>${label}</strong></td><td><strong>${money(s)}</strong></td><td class="negative">− ${money(e)}</td><td class="${s - e >= 0 ? 'positive' : 'negative'}"><strong>${s - e >= 0 ? '+ ' : '− '}${money(Math.abs(s - e))}</strong></td></tr>`;
  }).join('');
  const selectedStore = document.querySelector('#store-filter')?.value || 'all';
  const relevantStoreNames = selectedStore === 'all' ? Object.keys(cashHistoryByStore) : Object.keys(cashHistoryByStore).filter(name => name.toLowerCase() === selectedStore || name.toLowerCase().includes(selectedStore));
  const treasuryRows = relevantStoreNames.flatMap(storeName => (cashHistoryByStore[storeName] || []).filter(row => row.date_fermeture).map(row => ({ ...row, storeName }))).sort((a, b) => new Date(b.date_ouverture) - new Date(a.date_ouverture)).slice(0, 30).map(row => {
    const label = new Date(row.date_ouverture).toLocaleDateString('fr-FR', { weekday: 'short', day: '2-digit', month: 'short' });
    const ecart = row.ecart || 0;
    const theorique = row.solde_theorique ?? (row.montant_fermeture - ecart);
    const mouvement = theorique - row.montant_ouverture;
    return `<tr><td><strong>${label}</strong></td><td class="muted">${escapeHtml(row.storeName)}</td><td>${money(row.montant_ouverture)}</td><td class="${mouvement >= 0 ? 'positive' : 'negative'}">${mouvement >= 0 ? '+ ' : '− '}${money(Math.abs(mouvement))}</td><td>${money(theorique)}</td><td>${money(row.montant_fermeture)}</td><td class="${ecart >= 0 ? 'positive' : 'negative'}">${ecart >= 0 ? '+ ' : '− '}${money(Math.abs(ecart))}</td></tr>`;
  }).join('');
  return genericHeader('Rapports', 'Détail jour par jour des ventes, dépenses et trésorerie.', null, '') +
    `<section class="glass-card panel">
      <div class="panel-header"><div><h3>Ventes, dépenses et bénéfice net par jour</h3><p>Filtré selon le magasin sélectionné en haut</p></div></div>
      ${dailyRows ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Jour</th><th>Ventes</th><th>Dépenses</th><th>Bénéfice net</th></tr></thead><tbody>${dailyRows}</tbody></table></div>` : '<div class="empty-state"><strong>Aucune donnée</strong>Aucune vente ou dépense enregistrée pour ce filtre.</div>'}
    </section>
    <section class="glass-card panel" style="margin-top:14px">
      <div class="panel-header"><div><h3>Historique des caisses</h3><p>Ouvertures et fermetures passées</p></div></div>
      ${treasuryRows ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Journée comptable</th><th>Magasin</th><th>Ouverture</th><th>Mouvements</th><th>Solde théorique</th><th>Solde réel</th><th>Écart</th></tr></thead><tbody>${treasuryRows}</tbody></table></div>` : '<div class="empty-state"><strong>Aucun historique de caisse</strong>Ouvrez puis fermez une caisse pour voir son historique ici.</div>'}
    </section>`;
}
...
function initScrollReveal() {
  const targets = document.querySelectorAll('.glass-card:not(.stat-card), .cash-item');
...
function render() { const views = { dashboard: dashboardView, sales: salesView, cash: cashView, expenses: expensesView, collection: collectionView, products: productsView, team: teamView, reports: reportsView }; page.innerHTML = views[currentView](); document.querySelector('#breadcrumb-current').textContent = pageTitle(); document.querySelectorAll('.nav-item').forEach(item => item.classList.toggle('active', item.dataset.view === currentView)); bindViewEvents(); animateCounts(); initScrollReveal(); }
function openModal(type) { const modal = document.createElement('div'); modal.className = 'modal-backdrop'; let title = 'Nouvelle entrée'; let fields = ''; const storeOptions = stores.map(store => `<option value="${store.id}">${escapeHtml(store.nom)}</option>`).join(''); if (type === 'sale') { title = 'Enregistrer une vente'; const storeField = window.solmaPersonnelSession ? '' : `<div class="field"><label>Magasin</label><select id="modal-store"><option value="">Sélectionner un magasin</option>${storeOptions}</select></div>`; fields = `<div class="field"><label>Produit (optionnel)</label><input id="modal-product" placeholder="Nom du produit" value="${selectedProduct?.name || ''}" /></div><div class="field"><label>Montant (FCFA)</label><input id="modal-amount" type="number" placeholder="0" value="${selectedProduct?.price || ''}" /></div>${storeField}<div class="field"><label>Mode de paiement</label><select id="modal-payment"><option value="liquide">Liquide</option><option value="mobile_money">Mobile money</option></select></div>`; } else if (type === 'expense') { title = 'Ajouter une dépense'; const expenseStoreField = window.solmaPersonnelSession ? '' : `<div class="field"><label>Magasin</label><select id="modal-store">${storeOptions}</select></div>`; fields = `<div class="field"><label>Motif</label><input id="modal-reason" placeholder="Ex. Transport livraison" /></div><div class="field"><label>Montant (FCFA)</label><input id="modal-amount" type="number" placeholder="0" /></div>${expenseStoreField}`; } else if (type === 'product') { title = 'Ajouter un produit'; fields = `<div class="field"><label>Nom du produit</label><input id="modal-product" placeholder="Ex. Biscuit" /></div><div class="field"><label>Prix de vente (FCFA)</label><input id="modal-amount" type="number" placeholder="0" /></div><div class="field"><label>Magasin</label><select id="modal-store">${storeOptions}</select></div>`; } else { title = 'Ajouter une personne'; fields = `<div class="field"><label>Type de compte</label><select id="modal-role"><option value="vendeur">Vendeur</option><option value="admin">Administrateur</option></select></div><div class="field"><label>Nom complet</label><input id="modal-name" placeholder="Nom et prénom" /></div><div class="field vendeur-only"><label>Téléphone</label><input id="modal-phone" placeholder="77 000 00 00" /></div><div class="field vendeur-only"><label>Magasin</label><select id="modal-store">${storeOptions}</select></div><div class="field vendeur-only"><label>Code à 4 chiffres</label><input id="modal-pin" type="password" maxlength="4" placeholder="••••" /></div>`; }
...
async function cancelExpense(expenseId) { if (!confirm('Annuler cette dépense ? Elle restera visible dans l’historique.')) return; const session = (await window.solmaSupabase.auth.getSession()).data.session; if (!session) { showToast('Session admin expirée.'); return; } const result = await fetch('/api/expenses-delete', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ expense_id: expenseId }) }); if (!result.ok) { showToast('Impossible d’annuler cette dépense.'); return; } remoteDataLoaded = false; await loadRemoteData(); showToast('Dépense annulée.'); }
...
async function saveModal(type, modal) {
  ...
  if (type === 'team') {
    const role = modal.querySelector('#modal-role')?.value || 'vendeur';
    const payload = { nom: modal.querySelector('#modal-name').value.trim(), telephone: modal.querySelector('#modal-phone')?.value.trim(), pin: modal.querySelector('#modal-pin')?.value, magasin_id: storeId, role, email: modal.querySelector('#admin-personnel-email')?.value.trim(), password: modal.querySelector('#admin-personnel-password')?.value };
    if (role === 'admin') {
      if (!payload.nom || !payload.email || !payload.password || payload.password.length < 8) { showToast('Renseignez le nom, l’email et un mot de passe de 8 caractères minimum.'); return; }
    } else {
  ...
  if (type === 'expense') {
    ...
    const expenseStoreId = window.solmaPersonnelSession?.personnel?.magasin_id || storeId;
    if (!motif || amount <= 0 || !expenseStoreId) { showToast('Renseignez le motif et le montant.'); return; }
    const result = await browserFetch('/api/expenses', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ motif, montant: amount, magasin_id: expenseStoreId }) });
  ...
  if (type === 'sale') {
    const product = modal.querySelector('#modal-product').value.trim() || 'Vente rapide';
    const personnelId = window.solmaPersonnelSession?.personnel?.id || null;
    const saleStoreId = window.solmaPersonnelSession?.personnel?.magasin_id || modal.querySelector('#modal-store')?.value;
    const payment = modal.querySelector('#modal-payment')?.value || 'liquide';
    let productId = modal.querySelector('#modal-product').dataset.productId || null;
    if (amount <= 0) { showToast('Le montant est obligatoire.'); return; }
    if (!saleStoreId) { showToast('Sélectionnez un magasin.'); return; }
    if (window.solmaPersonnelSession && !personnelId) { showToast('Vendeur introuvable.'); return; }
    if (!productId && modal.querySelector('#add-to-catalog')?.checked) { const catalog = await window.solmaSupabase.from('produits').insert({ nom: product, prix: amount, magasin_id: saleStoreId }).select('id').single(); if (catalog.error && catalog.error.code !== '23505') { showToast('Impossible d’ajouter le produit au catalogue.'); return; } productId = catalog.data?.id || null; }
    const result = await browserFetch('/api/sales', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ nom_produit: product, montant: amount, mode_paiement: payment, magasin_id: saleStoreId, personnel_id: personnelId, produit_id: productId }) });
...
async function openCashModal(action, storeId, storeName) {
  const isOpen = action === 'open';
  let prefill = '';
  if (isOpen) {
    try {
      const token = window.solmaPersonnelSession?.token || (await window.solmaSupabase.auth.getSession()).data.session?.access_token;
      const result = await fetch(`/api/cash?magasin_id=${storeId}`, { headers: { Authorization: `Bearer ${token}` } });
      if (result.ok) { const payload = await result.json(); if (payload.cash?.date_fermeture) prefill = payload.cash.montant_fermeture; }
    } catch {}
  }
  const modal = document.createElement('div'); modal.className = 'modal-backdrop'; modal.innerHTML = `<div class="modal glass-card"><button class="modal-close" aria-label="Fermer">${icon('close', 16)}</button><p class="eyebrow">Caisse ${escapeHtml(storeName)}</p><h2>${isOpen ? 'Ouvrir la caisse' : 'Fermer la caisse'}</h2><p class="subtle">${isOpen ? (prefill !== '' ? `Montant de fermeture d'hier : ${money(prefill)}. Ajustez si besoin.` : 'Saisissez le montant réellement présent au démarrage.') : 'Comptez l’argent physiquement avant de confirmer.'}</p><div class="field"><label>Montant (FCFA)</label><input id="cash-amount" type="number" min="0" placeholder="0" value="${prefill}" /></div><div class="form-actions"><button class="btn btn-light modal-cancel">Annuler</button><button class="btn btn-primary modal-save">Confirmer</button></div></div>`;
  ...
  modal.querySelector('.modal-save').onclick = async () => {
    ...
    const token = window.solmaPersonnelSession?.token || (await window.solmaSupabase.auth.getSession()).data.session?.access_token;
    if (!Number.isInteger(amount) || amount < 0 || !token) { showToast('Montant invalide ou session expirée.'); return; }
    const result = await fetch('/api/cash', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ action, magasin_id: storeId, ...(isOpen ? { montant_ouverture: amount } : { montant_fermeture: amount }) }) });
    if (!result.ok) { showToast('Impossible de modifier la caisse.'); return; }
    close();
    remoteDataLoaded = false;
    cashHistoryByStore = {};
    if (window.solmaPersonnelSession) await loadPersonnelTreasury(); else await loadRemoteData();
    await loadCashHistory();
    showToast(isOpen ? 'Caisse ouverte.' : 'Caisse fermée.');
  };
}
document.addEventListener('click', event => {
  const reopenButton = event.target.closest('[data-action="reopen-cash"]');
  if (reopenButton) {
    event.stopImmediatePropagation();
    if (!confirm(`Rouvrir la dernière caisse de ${reopenButton.dataset.store} ? Utile si des ventes ont eu lieu après la fermeture.`)) return;
    (async () => {
      const cash = data.cash.find(item => item.store === reopenButton.dataset.store);
      const store = stores.find(item => item.nom === reopenButton.dataset.store);
      const storeId = cash?.id || store?.id;
      const token = window.solmaPersonnelSession?.token || (await window.solmaSupabase.auth.getSession()).data.session?.access_token;
      const result = await fetch('/api/cash', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ action: 'reopen', magasin_id: storeId }) });
      if (!result.ok) { showToast('Impossible de rouvrir la caisse.'); return; }
      remoteDataLoaded = false; cashHistoryByStore = {};
      if (window.solmaPersonnelSession) await loadPersonnelTreasury(); else await loadRemoteData();
      await loadCashHistory();
      showToast('Caisse rouverte.');
    })();
    return;
  }
  const button = event.target.closest('[data-action="open-cash"], [data-action="close-cash"]');
  if (!button) return;
  event.stopImmediatePropagation();
  const cash = data.cash.find(item => item.store === button.dataset.store);
  const store = stores.find(item => item.nom === button.dataset.store);
