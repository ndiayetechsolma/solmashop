...
function dashboardView() {
  ...
  const cashExpected = treasurySnapshot?.cash?.expected ?? (getFiltered(data.cash).filter(c => c.opened).reduce((sum, c) => sum + (c.opening || 0), 0) + todaySales.filter(sale => sale.payment === 'liquide').reduce((sum, sale) => sum + sale.amount, 0) - totalExpenses);
